...
**Scoring is table-driven.** The `ANIMALS` dict maps each image path to `{effect, value, legend}`, where `effect` is one of `points` / `obstacle` / `opponent` / `shield` / `random`. `collectAnimal(image_name, score, opponent_score, shield_active)` applies one collection and returns all three updated values — **callers must reassign all three**. Both players go through it, with the arguments swapped for player 2, so there is no per-player duplication. The start screen legend is drawn by looping over `legend_layout` (image path + two positions) and takes its label text from `ANIMALS`, so point values cannot drift out of step with the legend.

**To add an animal:** add an `ANIMALS` entry, add the path to `animal_images` or `rare_animal_images` (the spawn pools; rare spawns on a 1-in-8 roll in `summonAnimal`), and add a `legend_layout` row. A startup assertion fails loudly if a spawnable animal has no `ANIMALS` entry. Nothing is index-coupled any more, so reordering the pools is safe. `EAGLE` is a named constant because it is special-cased twice (faster `x_velocity`, recycled at `x_pos > 3200` rather than `> 1100`).

**Shields** are consumed in `checkForStar(shield_active, score, penalty)`: it returns the updated `(shield, score)` pair, absorbing the hit if the shield is up and otherwise applying `penalty` (a negative number). It is called only from `collectAnimal`, for `obstacle` animals, and the penalty comes from the `ANIMALS` table rather than being hardcoded.
