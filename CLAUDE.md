...
**Modes are table-driven.** `MODES` is a list of rules dicts — `name`, `tagline`, `point_goal`, `player_count`, `saves_highscore` — and the global `mode` points at the active one. The start screen cycles it with `M` (calling `newRound()` so the players list is rebuilt at the new seat count) and draws the mode's `tagline` as its third instruction line; `runGame()`/`runEndScreen()` read `mode["point_goal"]` for the win check, and the end screen only writes `Highscore.txt` when `saves_highscore` is true (solo times aren't comparable with two-player races). The chosen mode survives rounds and `K_r` restarts — nothing resets it. **A rule that varies by mode belongs in the `MODES` table, not in an `if` on the mode name**; per the roadmap, per-mode spawn sources/level scripts will hang off this table too.
