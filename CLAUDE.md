...
Pygame game in a single file, `main.py` (~400 lines), with two selectable modes: **Speed Run** (the original game — two local players, player 1 on WASD, player 2 on the arrow keys, first to 100 points wins) and **Adventure** (single player; the planned levels/hazards are not built yet, so for now it is a solo race to the point goal).

**Modes are table-driven.** `MODES` is a list of rules dicts — `name`, `tagline`, `point_goal`, `player_count`, `saves_highscore` — and the global `mode` points at the active one. The mode select screen picks it (up/down move the highlight, SPACE confirms and calls `newRound()` so the players list is rebuilt at the new seat count); the instructions screen then draws the mode's `name`, its `tagline` as the third instruction line, one controls entry per seat, and the high-score table only when `saves_highscore` is true. `runGame()`/`runEndScreen()` read `mode["point_goal"]` for the win check, and the end screen only writes `Highscore.txt` when `saves_highscore` is true (solo times aren't comparable with two-player races). The chosen mode survives rounds and `K_r` restarts — nothing resets it. **A rule that varies by mode belongs in the `MODES` table, not in an `if` on the mode name**; per the roadmap, per-mode spawn sources/level scripts will hang off this table too.
